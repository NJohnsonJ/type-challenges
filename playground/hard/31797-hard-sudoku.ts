...
type ContainsAllDigits<T extends number[], D extends number = Digits> =
    Exclude<D, T[number]> extends never ? true : false;

type GetRow<T extends Board, R extends number> = Flatten<T[R]>;

type GetColumn<T extends Board, C extends number> = [GetRow<T, 0>[C], GetRow<T, 1>[C], GetRow<T, 2>[C], GetRow<T, 3>[C], GetRow<T, 4>[C], GetRow<T, 5>[C], GetRow<T, 6>[C], GetRow<T, 7>[C], GetRow<T, 8>[C]];
type GetColumns<T extends Board> = [GetColumn<T, 0>, GetColumn<T, 1>, GetColumn<T, 2>, GetColumn<T, 3>, GetColumn<T, 4>, GetColumn<T, 5>, GetColumn<T, 6>, GetColumn<T, 7>, GetColumn<T, 8>];

type AllColumnsSolved<T extends Board> = ContainsAllDigits<GetColumns<T>[number]>;

type RowSolved<T extends Row> = ContainsAllDigits<Flatten<T>>;
type AllRowsSolved<T extends Board> = RowSolved<T[number]>;

type And<T extends boolean[]> =
    T extends [infer Head, ...infer Tail extends boolean[]]
        ? Head extends false ? false : And<Tail> : true;

type SudokuSolved<T extends Board> = And<[AllRowsSolved<T>, AllColumnsSolved<T>]>;

type A = GetColumns<Sudoku8>;
...
type test_sudoku_1_actual = SudokuSolved<Sudoku1>
...
type test_sudoku_8_actual = SudokuSolved<Sudoku8>
