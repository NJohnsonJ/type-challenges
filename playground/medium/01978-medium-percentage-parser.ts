...
type ParseSign<T extends string> = T extends `${infer Sign extends '+' | '-'}${string}` ? Sign : '';
type ParsePercent<T extends string> = T extends `${string}%` ? '%' : '';
type PercentageParser<A extends string, Sign extends string = ParseSign<A>, Percent extends string = ParsePercent<A>> =
    A extends `${Sign}${infer R}${Percent}` ? [Sign, R, Percent] : never;
