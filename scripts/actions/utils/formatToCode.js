...
export function formatToCode(quiz, locale) {
    return `${toCommentBlock(toInfoHeader(quiz, locale)
        + (quiz.readme[locale] || quiz.readme[defaultLocale])
        + toLinks(quiz, locale))
        + toDivider(t(locale, 'divider.code-start'))}\n${(quiz.template || '').trim()}\n${toDivider(t(locale, 'divider.test-cases'))}${(quiz.tests || '').trim()}\n${toDivider(t(locale, 'divider.further-steps'))}${toCommentBlock(toFooter(quiz, locale))}`;
}
