...
export const getOthers = (condition, a, b) => condition ? a : b;
const action = async (github, context, core) => {
    ...
    if (labels.includes('new-challenge')) {
        ...
        const playgroundBadge = toBadgeLink(playgroundURL, '', t(locale, 'badge.preview-playground'), '3178c6', '?logo=typescript&logoColor=white');
...
export function getQuestionFullName(no, difficulty, title) {
    return `${String(no).padStart(5, '0')}-${difficulty}-${slug(title.replace(/\./g, '-').replace(/<.*>/g, ''), { tone: false })}`;
}
